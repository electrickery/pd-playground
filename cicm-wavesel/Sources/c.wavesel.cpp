...
typedef struct  _wavesel
{
    ...
    t_float     x_lastYCoordinate;
    int         x_clipmode;
    int         x_selectmode;
    
    t_float     selStartCursor;
    t_float     selEndCursor;
    t_float     selOriginCursor;
...
static void wavesel_draw_waveform(t_wavesel *x, t_rect *rect)
{
    ...
    outlet_float(x->f_out_1, 0 / x->x_ksr);
    outlet_float(x->f_out_2, x->x_arraysize / x->x_ksr);
...
static void wavesel_draw_selection(t_wavesel *x, t_rect *rect)
{
    ...
    if(g)
    {
        ...
        float start = (x->selEndCursor > x->selStartCursor) ? x->selStartCursor : x->selEndCursor;
        float end = (x->selEndCursor > x->selStartCursor)   ? x->selEndCursor   : x->selStartCursor;
...
//        egraphics_set_color_rgba(g, &color);
        egraphics_set_color_rgba(g, &x->f_color_selbground);
...
static void wavesel_paint(t_wavesel *x, t_object *view)
{
    ...
    if(x->f_mode != wavesel_sym_none)
    {
        wavesel_draw_selection(x, &rect);
    }
}

static void wavesel_center_selection(t_wavesel *x, int pointerPosition)
{
    t_float halfWidth = (x->selEndCursor - x->selStartCursor) / 2;
    t_rect rect;
    ebox_get_rect_for_view((t_ebox *)x, &rect);
    if (x->highlightMode)
    {
        x->selStartCursor = pointerPosition - halfWidth;
        x->selStartCursor = (x->selStartCursor < 0) ? 0 : x->selStartCursor;
        x->selEndCursor   = pointerPosition + halfWidth;
        x->selEndCursor   = (x->selEndCursor > rect.width) ? rect.width : x->selEndCursor;
    }
    else
    {
        x->selStartCursor = pointerPosition;
        x->selEndCursor   = pointerPosition;
    }
}

static void wavesel_resize_selection(t_wavesel *x, t_float pointerPosition)
{
    t_rect rect;
    ebox_get_rect_for_view((t_ebox *)x, &rect);
    ...
    {
        t_float mid = (x->selStartCursor + x->selEndCursor) / 2;
        x->selStartCursor = mid;
        x->selEndCursor = mid;
    }
}

static void wavesel_click_select(t_wavesel *x, t_pt pt)
{
    outlet_float(x->f_out_3, pt.x * x->x_outputFactor);
    outlet_float(x->f_out_4, pt.x * x->x_outputFactor);
    x->selStartCursor = pt.x;
    x->selEndCursor   = pt.x;
    x->highlightMode  = 1;
}

static void wavesel_click_loop(t_wavesel *x, t_pt pt)
{
    if (x->highlightMode)
    {
        wavesel_center_selection(x, pt.x);
        
        outlet_float(x->f_out_3, x->selStartCursor * x->x_outputFactor);
        outlet_float(x->f_out_4, x->selEndCursor * x->x_outputFactor);  
    }  
    else
    {
        outlet_float(x->f_out_3, pt.x * x->x_outputFactor);
...
static void wavesel_mousedown(t_wavesel *x, t_object *patcherview, t_pt pt, long modifiers)
{
    ...
    {
        case 1:
            wavesel_click_select(x, pt);
            x->highlightMode = 1;
            break;
         case 2:
            wavesel_click_loop(x, pt);
            break;
        case 3:
        post("x: %f, y: %f", pt.x, pt.y);
//            wavesel_click_move(x, pt);
//            break;
        case 4:
//            wavesel_click_draw(x, pt);
//            break;
        default:
            break;
    }

    x->selOriginCursor = pt.x;
    ebox_invalidate_layer((t_ebox *)x, wavesel_sym_selection_layer);
    ebox_redraw((t_ebox *)x);
    if (patcherview && modifiers) {} // suppress compiler warnings
}

static void wavesel_motion_select(t_wavesel *x, t_pt pt)
{
    t_rect rect;
    ebox_get_rect_for_view((t_ebox *)x, &rect);
    
    if (pt.x > x->selOriginCursor)
    {
        x->selEndCursor = pt.x;
        x->selEndCursor = (x->selEndCursor > rect.width) ? rect.width : x->selEndCursor;
        outlet_float(x->f_out_4, x->selEndCursor * x->x_outputFactor);
...
static void wavesel_motion_loop(t_wavesel *x, t_pt pt)
{
    t_rect rect;
    ebox_get_rect_for_view((t_ebox *)x, &rect);
    
    wavesel_center_selection(x, pt.x);
    if (x->x_lastYCoordinate)
        wavesel_resize_selection(x, x->x_lastYCoordinate - pt.y);
    x->x_lastYCoordinate = pt.y;
...
static void wavesel_mousedrag(t_wavesel *x, t_object *patcherview, t_pt pt, long modifiers)
{
    ...
    {
        case 1:
            wavesel_motion_select(x, pt);
            break;
         case 2:
            wavesel_motion_loop(x, pt);
            break;
        case 3:
            post("x: %f, y: %f", pt.x, pt.y);
//            wavesel_click_move(x, pt);
...
//        x->x_arraysize = 0;
    ...
    x->selEndCursor = x->selStartCursor = 0.f;
    x->highlightMode = 0;
...
static void wavesel_state(t_wavesel *x)
{
    ...
    post("sel_start_cursor: %f",    x->selStartCursor);
    post("sel_end_cursor: %f",      x->selEndCursor);
    post("mode: %s",                x->f_mode->s_name);
    post("mode: %d",                x->x_mode);
    post("highlightMode: %d",       x->highlightMode);
/*    post("-");
...
// workaround until wavesel_mode_set will work
static void wavesel_setmode(t_wavesel *x, t_float m)
{
    x->x_mode = (int)m;
    x->x_mode = (x->x_mode < 0) ? 0 : x->x_mode;
    x->x_mode = (x->x_mode > 5) ? 5 : x->x_mode;
    switch (x->x_mode)
    {
        case 0:
...
static void *wavesel_new(t_symbol *s, int argc, t_atom *argv)
{
    ...
    if(x && d)
    {
        ...
        x->x_selectmode = 0;
        x->f_mode           = wavesel_sym_select;
        x->selStartCursor = 0.f;
        x->selEndCursor   = 0.f;
        x->x_lastYCoordinate = 0.f;
        x->highlightMode   = 0;
...
extern "C" void setup_c0x2ewavesel(void)
{
    ...
    CLASS_ATTR_DEFAULT_SAVE_PAINT   (c, "bdcolor", 0, "0.0 0.0 0.0 1.");
    ...
    CLASS_ATTR_DEFAULT_SAVE_PAINT   (c, "secolor", 0, "0.3 1.0 0.3 1.");
    CLASS_ATTR_STYLE                (c, "secolor", 0, "color");

    CLASS_ATTR_RGBA                 (c, "sbcolor", 0, t_wavesel, f_color_selbground);
    CLASS_ATTR_LABEL                (c, "sbcolor", 0, "Selection Background Color");
    CLASS_ATTR_ORDER                (c, "sbcolor", 0, "5");
    CLASS_ATTR_DEFAULT_SAVE_PAINT   (c, "sbcolor", 0, "0.0 0.0 0.0 1.");
    CLASS_ATTR_STYLE                (c, "sbcolor", 0, "color");
